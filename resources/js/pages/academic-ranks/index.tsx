...
import { AcademicRankModalFormConfig } from '@/config/forms/academic-rank-modal-form'
import { AcademicRankTableConfig } from '@/config/tables/academic-rank-table'
...
interface IndexProps {
  academicRanks: Pagination<AcademicRank>
  filters?: FilterProps
}
...
export default function AcademicRankIndex({ academicRanks, filters }: IndexProps) {
  ...
  const [selectedItemForView, setSelectedItemForView] = useState<AcademicRank | null>(null)
  ...
  const openModal = (nextMode: 'create' | 'view' | 'edit', item?: AcademicRank) => {
    if (nextMode === 'view') {
      handleViewItem(item)
      return
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const routePath = isEditMode
      ? route('academic-ranks.update', { academic_rank: selectedItem!.id })
      : route('academic-ranks.store')
  ...
  const handleRestore = (id: string | number) => {
    router.post(route('academic-ranks.restore', id), {}, {
      preserveScroll: true,
      onSuccess: () => triggerFetch({}),
      onError: () => toast.error('Failed to restore academic rank'),
    })
  }
  ...
  const tableData = academicRanks.data.map((item) => ({
    ...item,
  ...
  const triggerFetch = (params: Record<string, any> = {}) => {
    ...
    router.get(route('academic-ranks.index'), {
  ...
  const from = academicRanks?.from ?? 0
  const total = academicRanks?.total ?? 0
  const currentPage = academicRanks?.meta?.current_page || 1
  const lastPage = academicRanks?.meta?.last_page || 1
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="Academic Ranks" />
      <CustomToast />

      <PageLayout
        title="Academic Ranks"
        subtitle={showDeleted ? "Viewing deleted academic ranks. You can restore or permanently delete them." : "Manage academic ranks for faculty positions (e.g., Instructor, Professor)."}
        primaryAction={{
          label: 'Add Academic Rank',
          icon: <Plus className="h-4 w-4" />,
          onClick: () => openModal('create'),
          permission: hasPermission(permissions, 'create-academic-rank'),
        }}
        searchValue={searchTerm}
        onSearchChange={handleSearchChange}
        isSearching={isSearching}
        searchPlaceholder="Search academic ranks..."
        ...
        actionsSlot={
          <>
            <div className="flex items-center gap-1">
              ...
              {(hasPermission(permissions, 'restore-academic-rank') || hasPermission(permissions, 'force-delete-academic-rank')) && (
                <IconButton
        ...
        {showDeleted && (
          <div className="mb-3 px-3 md:px-6">
            <div className="flex items-center gap-2 text-sm text-amber-700 dark:text-amber-400">
              ...
              <span>Viewing deleted academic ranks</span>
            </div>
          </div>
        )}
        <EnterpriseEmployeeTable
          columns={AcademicRankTableConfig.columns}
          actions={AcademicRankTableConfig.actions}
          data={tableData}
          from={academicRanks.from}
          onDelete={handleDelete}
          onView={handleViewItem}
          onEdit={(item) => openModal('edit', item)}
          onRestore={handleRestore}
          onForceDelete={handleForceDelete}
          resourceType="academic-rank"
          enableExpand={false}
          viewMode="table"
        />
      </PageLayout>
      ...
      <CustomModalForm
        ...
        addButton={{ ...AcademicRankModalFormConfig.addButton, label: '', className: 'h-9 w-9 p-0' }}
        title={mode === 'view' ? 'View Academic Rank' : mode === 'edit' ? 'Update Academic Rank' : AcademicRankModalFormConfig.title}
        description={AcademicRankModalFormConfig.description}
        fields={AcademicRankModalFormConfig.fields}
        buttons={AcademicRankModalFormConfig.buttons}
        data={data}
        setData={setData}
        errors={errors}
